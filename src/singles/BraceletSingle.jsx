...
import { braceletData } from '../jewellery/data/bracelet'

const BraceletSingle = () => {
    const { category,id } = useParams()
    const { toggleWishlistItem, wishlistItems } = useWishlist()
    const { addToCart } = useCart()

    // 1. Check ID type in your data first
    const product = braceletData.find(item => 
    ...
    if (!product) {
        ...
        console.log('Available IDs:', braceletData.map(item => item.id))
    ...
    return (
        ...
        <div className="single-heart">
          <i 
          onClick={() => toggleWishlistItem({...product, category: 'bracelet'})}
